                ...
                <div class="list-group mx-2">
                    <h6>
                        ...
                        @foreach ($catlist as $cat)
                            <a class="text-decoration-none list-group-item"
                                href="{{ route('categories.index', ['id_category' => $cat->id]) }}"> <img
                                    src="{{ url($cat->icon) }}" style="width: 30px; height: 30px" alt="">
                                {{ $cat->name }}</a>
                        @endforeach
                    </h6>
                </div>
                            ...
                            <div class="text-center d-flex gap-5">
                                <form action="{{ route('cart.store') }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-outline-primary"
                                        @if ($prod->quantity == 0) disabled @endif>Comprar</button>
                                ...
                                <a href="{{ route('products.show', ['product' => $prod->id]) }}"> <button type="button"
